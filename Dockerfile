...
WORKDIR /workspaces/orghi-memory/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/orghi-memory/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/orghi_memory/ && touch src/orghi_memory/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/orghi-memory/
...
ENTRYPOINT ["/opt/orghi-memory-env/bin/poe"]
CMD ["api"]
